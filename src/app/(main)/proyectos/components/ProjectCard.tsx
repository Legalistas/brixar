...
import { Calendar, Building2, Home, Maximize, type LucideIcon } from 'lucide-react';
...
const ProjectCard = ({ project }: ProjectCardProps) => {
    ...
    const ProjectTypeIcon = projectTypeIcons[project.proyectDetails.type] || Building2;
    ...
    return (
        <div className="bg-white rounded-lg overflow-hidden shadow border border-gray-200 transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-lg flex flex-col h-full">
            ...
            <div className="relative h-48 w-full flex-shrink-0">
                <div className={`absolute top-0 left-0 z-10 px-3 py-1 text-xs font-semibold text-white rounded-br-lg bg-gradient-to-r ${getPhaseColor(project.phase)}`}>
                    {project.phase}
                </div>
                <Image
            ...
            <div className="p-4 flex-grow flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                    <div className="flex items-center gap-1 text-gray-600">
                        ...
                        <span className="text-xs">Año {project.proyectDetails.buildingYear}</span>
                    </div>

                    <div className="flex items-center gap-1 text-gray-600">
                        <ProjectTypeIcon className="h-4 w-4" />
                        <span className="text-xs">{formatProjectType(project.proyectDetails.type)}</span>
                    </div>

                    {project.proyectDetails.surface && (
                        <div className="flex items-center gap-1 text-gray-600">
                            <Maximize className="h-4 w-4" />
                            <span className="text-xs">{project.proyectDetails.surface} m²</span>
                        </div>
                    )}
                </div>
            ...
            <div className="p-4 border-t border-gray-200 mt-auto">
                <Link href={`/proyectos/${project.slug}`} className="p-2 text-xs font-medium block text-center rounded-md bg-[#FB6107] hover:bg-[#FB6107]/90 text-white transition-colors">
                    Ver detalles
                </Link>
            </div>
        </div>
