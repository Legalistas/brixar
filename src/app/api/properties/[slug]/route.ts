...
export async function PATCH(
  ...
  try {
    ...
    // Manejar las imágenes correctamente
    if (images && Array.isArray(images)) {
      ...
      // Verificar si las imágenes son strings (URLs) o objetos
      const imageEntries = images.map((img) => {
        // Si la imagen es un string (URL), crear un objeto con solo la URL
        if (typeof img === 'string') {
          return { url: img }
        }
        // Si la imagen ya es un objeto, extraer solo la URL
        return { url: img.url }
      })

      // Filtrar imágenes inválidas
      const validImages = imageEntries.filter(
