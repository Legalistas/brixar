...
import { motion, useInView, useMotionValue, MotionValue } from 'framer-motion'

import { AppScreen } from '@/components/AppScreen'
import { ChartProps, Point } from '@/types/app-demo'
...
function Chart({
    ...
    const width: number = totalWidth - paddingX * 2
    const height: number = totalHeight - paddingY * 2

    const id: string = useId()
    const svgRef = useRef<SVGSVGElement>(null)
    const pathRef = useRef<SVGPathElement>(null)
    const isInView: boolean = useInView(svgRef, { amount: 0.5, once: true })
    const pathWidth: MotionValue<number> = useMotionValue(0)
    ...
    for (let index = 0; index < prices.length; index++) {
        const x: number = paddingX + (index / (prices.length - 1)) * width
        const y: number =
            paddingY +
...
export function AppDemo() {
    ...
    const activePriceIndex: number = activePointIndex ?? prices.length - 1
    const activeValue: number = prices[activePriceIndex]
    const previousValue: number = prices[activePriceIndex - 1]
    const percentageChange: number | null =
        activePriceIndex === 0
            ? null
